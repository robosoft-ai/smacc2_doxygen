...
\DoxyCodeLine{46     RCLCPP\_WARN\_STREAM(rclcpp::get\_logger(\textcolor{stringliteral}{"{}smacc2"{}}), \textcolor{stringliteral}{"{}[ClRosLaunch] starting ros launch thread "{}});}
\DoxyCodeLine{47     std::stringstream cmd;}
\DoxyCodeLine{48     cmd << \textcolor{stringliteral}{"{}ros2 launch "{}} << packageName << \textcolor{stringliteral}{"{} "{}} << launchFileName;}
\DoxyCodeLine{49 }
\DoxyCodeLine{50     std::array<char, 128> buffer;}
\DoxyCodeLine{51     std::string result;}
\DoxyCodeLine{52     std::unique\_ptr<FILE, decltype(\&pclose)> pipe(popen(cmd.str().c\_str(), \textcolor{stringliteral}{"{}r"{}}), pclose);}
\DoxyCodeLine{53     \textcolor{keywordflow}{if} (!pipe)}
\DoxyCodeLine{54     \{}
\DoxyCodeLine{55       \textcolor{keywordflow}{throw} std::runtime\_error(\textcolor{stringliteral}{"{}popen() failed!"{}});}
\DoxyCodeLine{56     \}}
\DoxyCodeLine{57 }
\DoxyCodeLine{58     std::stringstream ss;}
\DoxyCodeLine{59     \textcolor{keywordtype}{bool} cancelled = \textcolor{keyword}{false};}
\DoxyCodeLine{60     \textcolor{keywordflow}{while} (fgets(buffer.data(), buffer.size(), pipe.get()) != \textcolor{keyword}{nullptr} \&\&}
\DoxyCodeLine{61            !(cancelled = cancelCondition()))}
\DoxyCodeLine{62     \{}
\DoxyCodeLine{63       ss << buffer.data();}
\DoxyCodeLine{64     \}}
\DoxyCodeLine{65 }
\DoxyCodeLine{66     result = ss.str();}
\DoxyCodeLine{67     RCLCPP\_WARN\_STREAM(rclcpp::get\_logger(\textcolor{stringliteral}{"{}smacc2"{}}), \textcolor{stringliteral}{"{}[ClRosLaunch] RESULT = \(\backslash\)n "{}} << ss.str());}
