...
\DoxyCodeLine{47       rclcpp::get\_logger(\textcolor{stringliteral}{"{}smacc2"{}}), \textcolor{stringliteral}{"{}[ClRosLaunch static] starting ros launch thread "{}});}
\DoxyCodeLine{48 }
\DoxyCodeLine{49     std::stringstream cmd;}
\DoxyCodeLine{50     cmd << \textcolor{stringliteral}{"{}ros2 launch "{}} << packageName << \textcolor{stringliteral}{"{} "{}} << launchFileName;}
\DoxyCodeLine{51 }
\DoxyCodeLine{52     std::array<char, 128> buffer;}
\DoxyCodeLine{53     std::string result;}
\DoxyCodeLine{54     std::unique\_ptr<FILE, decltype(\&pclose)> pipe(popen(cmd.str().c\_str(), \textcolor{stringliteral}{"{}r"{}}), pclose);}
\DoxyCodeLine{55     \textcolor{keywordflow}{if} (!pipe)}
\DoxyCodeLine{56     \{}
\DoxyCodeLine{57       \textcolor{keywordflow}{throw} std::runtime\_error(\textcolor{stringliteral}{"{}popen() failed!"{}});}
\DoxyCodeLine{58     \}}
\DoxyCodeLine{59 }
\DoxyCodeLine{60     std::stringstream ss;}
\DoxyCodeLine{61     \textcolor{keywordtype}{bool} cancelled = \textcolor{keyword}{false};}
\DoxyCodeLine{62     \textcolor{keywordflow}{while} (fgets(buffer.data(), buffer.size(), pipe.get()) != \textcolor{keyword}{nullptr} \&\&}
\DoxyCodeLine{63            !(cancelled = cancelCondition()))}
\DoxyCodeLine{64     \{}
\DoxyCodeLine{65       ss << buffer.data();}
\DoxyCodeLine{66     \}}
\DoxyCodeLine{67 }
\DoxyCodeLine{68     result = ss.str();}
\DoxyCodeLine{69     RCLCPP\_WARN\_STREAM(}
\DoxyCodeLine{70       rclcpp::get\_logger(\textcolor{stringliteral}{"{}smacc2"{}}), \textcolor{stringliteral}{"{}[ClRosLaunch static]] RESULT = \(\backslash\)n "{}} << ss.str());}
